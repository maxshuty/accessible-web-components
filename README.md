...
## Setting the slider min/max values dynamically using JavaScript
...
**NOTE:** You must provide a `sliderId`, `minValue`, and `maxValue` in the `event.detail`. If any of these three parameters are not included in the event then it will throw an error in your console and ignore the event.
